...
class OutputLayer(torch.nn.Module):
    ...
    def forward(self, model_encoder_outputs, mask):
        ...
        logits_start = self.start_span_linear(
            torch.cat((model_encoder_outputs[0], model_encoder_outputs[1]), dim=-1))
        logits_end = self.start_span_linear(
            torch.cat((model_encoder_outputs[0], model_encoder_outputs[2]), dim=-1))
...
class SelfAttention(torch.nn.Module):
    ...
    def __init__(self, hidden_size, num_attn_heads):
        ...
        self.att = nn.MultiheadAttention(
            hidden_size, num_attn_heads, bias=False)

        self.w_q = nn.Linear(in_features=hidden_size,
                             out_features=hidden_size, bias=False)
        self.w_k = nn.Linear(in_features=hidden_size,
                             out_features=hidden_size, bias=False)
    ...
    def forward(self, x, mask=None):
        ...
        x = self.att(self.w_q(x), self.w_k(
            x), self.w_v(x), key_padding_mask=~mask)[0]
...
class EncoderBlock(torch.nn.Module):
    ...
    def __init__(self, hidden_size, device, num_convs, num_attn_heads, kernel_size, drop_prob):
        ...
        self.convs = ModuleList([DepthwiseSeparableConv1d(in_channels=hidden_size, out_channels=hidden_size,
    ...
    def forward(self, x, x_mask):
        ...
        for i, conv in enumerate(self.convs):
            ...
            # by transposing it, we get (batch_size, hidden_size, seq_len). Looking at the conv1d docs, this makes our in_channels equal to hidden_size as desired.
            output = conv(output.transpose(-1, -2)).transpose(-1, -2)
...
class PositionEncoder(torch.nn.Module):
    def __init__(self, hidden_size, device, drop_prob, max_seq_len=1000):
        ...
        # shape (max_seq_len, hidden-size // 2). These are the t values in p_t
        positions = positions.repeat(1, hidden_size // 2)

        self.position_encodings = torch.zeros(
            (max_seq_len, hidden_size)).to(device)
...
class Embedding(nn.Module):
    ...
    def __init__(self, word_vectors, character_vectors, hidden_size, drop_prob):
        ...
        self.drop_prob = drop_prob
        self.char_embed = nn.Embedding.from_pretrained(
            character_vectors, freeze=False)
        ...
        self.conv = nn.Conv2d(
            in_channels=character_vectors.shape[-1], out_channels=hidden_size, kernel_size=(1, 5))
        self.conv_projection = nn.Conv1d(
            in_channels=word_vectors.shape[-1] + hidden_size, out_channels=hidden_size, kernel_size=1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        # shape (batch_size, hidden_size, seq_len, word_len)
        ...
        # shape (batch_size, seq_len, word_embed_size)
        word_emb = F.dropout(word_emb, self.drop_prob, self.training)

        # (batch_size, seq_len, word_emb_size hidden_size)
