...
class QANet(nn.Module):
    ...
    def __init__(self, word_vectors, character_vectors, hidden_size, device, drop_prob, num_enc_blocks=[1, 5]):
        ...
        self.att = LSHSelfAttention(dim=hidden_size, dropout=drop_prob)
    ...
    def forward(self, cw_idxs, cc_idxs, qw_idxs, qc_idxs):
        ...
        for i in range(len(self.emb_encs)):
            ...
            c_enc = self.emb_encs[i](c_enc, c_mask)
            # (batch_size, max_query_len, hidden_size)
            q_enc = self.emb_encs[i](q_enc, q_mask)
        ...
        for _ in range(3):
            for i in range(len(self.mod_encs)):
                # (batch_size, c_len, 4 * hidden_size)
                mod = self.mod_encs[i](mod, c_mask)
            mods.append(mod)
